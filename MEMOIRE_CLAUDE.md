...
- start.py: cache-bust reapplique si query manquante meme avec marker present.
...
## MAJ 2026-01-01 - Debug index/auto-refresh (non resolu)

Ce qui a ete fait:
- start.py: injection BACKEND_BASE_URL avant le bundle JS, + cache-bust JS/CSS pour forcer le reload.
- UI Market Trends: essais auto-refresh successifs:
  - sleep + st.rerun (bloque le rendu, boucle).
  - components.html (reload JS non bloquant) -> React #321 persiste.
  - st.fragment(run_every=60) -> React #321 persiste.
  - streamlit-autorefresh -> retire (dep inutile).
  - meta refresh HTML (sans widget) -> toujours instable.
- CSX index: fallback persistant (session + fichier) + override env (CSX_INDEX_FALLBACK_*).
- Health monitor: ajout grace period + readiness event pour eviter API kill pendant startup.
- Verifs: /_stcore/health = 200 ; /Market_Trends/_stcore/* = 404 (attendu).

Etat actuel:
- Erreur React #321 (invalid hook call) toujours visible en prod (index.3bHSf9gi.js).
- Auto-refresh pas fiable (page boucle ou erreur client).
- "Connection refused" arrive quand l API est tuee trop tot; le grace period est en place mais a valider.

Pistes pour reprise:
- Tester sans cache-bust query (retirer ?v=codex-baseurl-1) si la duplication de bundle provoque #321.
- Forcer un hard refresh / clear site data pour vider les anciens bundles.
- Si #321 persiste, desactiver auto-refresh UI et passer par un job serveur (daily) + bouton manuel.
- Verifier qu une seule version de Streamlit JS est chargee (pas de mix bundle avec/sans query).
